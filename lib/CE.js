export class CE {
    ...
    def(args) {
        ...
        class newClass extends ext {
            ...
            detachQR() {
                ...
                doActions(self, syncActionsToDo, this, propChangeQueue);
                doActions(self, asyncActionsToDo, this, propChangeQueue);
    ...
    async doActions(self, actions, target, arg) {
        for (const methodName in actions) {
            ...
            if (action.debug)
                debugger;
            const ret = action.async ? await target[methodName](target) : target[methodName];
            if (ret === undefined)
                continue;
