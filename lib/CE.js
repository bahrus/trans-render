export class CE {
    ...
    async doActions(self, actions, target, arg) {
        for (const methodName in actions) {
            const fn = target[methodName].bind(target);
    ...
    getProps(action) {
        return new Set([...(action.ifAllOf || []), ...(action.actIfKeyIn || []), ...(action.andAlsoActIfKeyIn || [])]);
    }
