...
let count = 0;
const map = new Map();
const alreadyGroomed = new WeakSet();
export function groom(templ) {
    if (alreadyGroomed.has(templ))
        return;
...
function passVal(val, el, enh) {
    const enhancement = lispToCamel(enh);
    if (el.beEnhanced === undefined)
        el.beEnhanced = {};
    const { beEnhanced } = el;
    beEnhanced[enh] = val;
}
export async function restore(clone) {
            ...
            switch (key) {
                ...
                default:
                    ...
                    if (customElements.get(tagName)) {
                        await el.beEnhanced.whenResolved(tagName);
                    }
            }
        }
    }
