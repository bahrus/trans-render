...
export class TR {
    ...
    async transform(fragment) {
        ...
        for (const key in match) {
            ...
            let matches = (isArray ?
                fragment.filter(x => x.matches(query))
                : fromCache || (matchMap[key] = Array.from(fragment.querySelectorAll(query)).map(el => new WeakRef(el))));
    ...
    async do_object({ target, rhs, host, match, queryInfo }) {
