...
export class TR {
    ...
    async transform(fragment, fragmentManager) {
        ...
        let clearHost = false;
        if (fragment.host === undefined && fragment instanceof DocumentFragment) {
                    ...
                    if (fromCache) {
                        ...
                        switch (verbx) {
                            case 'querySelector':
                                ...
                                break;
                            default:
                                matches = matchMap[key] = Array.from(fragment.querySelectorAll(query)).map(el => new WeakRef(el));
                        }
                    }
                }
            }
    ...
    getDefaultProp(target) {
        if ('href' in target)
            return 'href';
        if ('value' in target && !('button-li'.includes(target.localName)))
            return 'value';
        return 'textContent';
    }
    async do_string(ctx) {
